              ...
              <div className="flex-1 text-left rtl:text-right">
                ...
                <div className="mt-2.5">
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold border transition-all cursor-pointer select-none shadow-xs ${
                  ...
                  >
                    <Phone className={`w-3.5 h-3.5 shrink-0 ${dropoffAddress.includes("Call recipient on arrival") ? 'text-[#cca073]' : 'text-emerald-700'}`} />
        ...
        <section>
          ...
          <div className="flex gap-3 overflow-x-auto hide-scrollbar pb-2 -mx-6 px-6">
            {[
              ...
              >
                <span className={`text-[12px] font-bold tracking-widest uppercase ${d.active ? 'text-[#a5b994]' : ''}`}>{d.day}</span>
                <span className={`text-2xl font-black ${d.active ? 'text-white' : 'text-zinc-900 dark:text-zinc-100'}`}>{d.date}</span>
                <span className={`text-[12px] font-bold tracking-widest uppercase ${d.active ? 'text-[#a5b994]' : ''}`}>{d.month}</span>
              </button>
            ))}
          </div>
